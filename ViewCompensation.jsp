        ...
        <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark" id="sidenav-main">
            ...
            <div class="collapse navbar-collapse  w-auto  max-height-vh-100" id="sidenav-collapse-main">
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-white active bg-gradient-primary" href="ViewCompensation.jsp">
                            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
                                <i class="material-icons opacity-10">table_view</i>
                            </div>
            ...
            <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
                <div class="container-fluid py-1 px-3">
                    <nav aria-label="breadcrumb">
                        ...
                        <h6 class="font-weight-bolder mb-0">Bồi thường</h6>
                    </nav>
          ...
          <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3">Chi tiết bồi thường</h6>
              </div>
            </div>
              ...
              <div class="table-responsive p-0">
                <table class="table align-items-center mb-0">
                  <thead>
                    <tr>
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Mã hồ sơ bồi thường</th>
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Mô tả</th>
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Ngày lập</th>
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Trạng thái</th>
                      <th class="text-secondary opacity-7"></th>
                    </tr>
                  </thead>
                  <c:forEach items="${a.compensation}" var="compen">
                      <c:choose>
                          <c:when  test = "${compen.userId == sessionScope.acc.userId}">
                  <tbody>
                    <tr>
                      <td>
                        <div class="d-flex px-2 py-1">
                          <div class="d-flex flex-column justify-content-center">
                            <h6 class="mb-0 text-sm">${compen.id}</h6>
                          </div>
                        </div>
                      </td>
                      <td>
                        <p class="text-xs font-weight-bold mb-0">${compen.description}</p>
                      </td>
                      
                      <td class="align-middle text-center">
                        <span class="text-secondary text-xs font-weight-bold">${compen.date}</span>
                      </td>
                      <td class="align-middle text-center text-sm">
                          <c:choose>
                          <c:when  test = "${compen.status == 'true'}">
                        <span class="badge badge-sm bg-gradient-success">${compen.status == "true" ? "Còn hiệu lực" : "Hết hiệu lực"}</span>
                        </c:when>
                        <c:otherwise>
                            <span class="badge badge-sm bg-gradient-secondary">${compen.status == "true" ? "Còn hiệu lực" : "Hết hiệu lực"}</span>
                        </c:otherwise>
                          </c:choose>
                          </td>
